"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var ng2_slim_loading_bar_1 = require("ng2-slim-loading-bar");
var InfoWrapperComponent = (function () {
    function InfoWrapperComponent(route, slimBarService) {
        this.route = route;
        this.slimBarService = slimBarService;
        this.lang = "zh";
    }
    InfoWrapperComponent.prototype.ngOnInit = function () {
        this.route.queryParams.subscribe(this.resolveQuery.bind(this));
    };
    InfoWrapperComponent.prototype.resolveQuery = function (query) {
        this.slimBarService.start();
        this.lang = query["lang"];
        this.pageType = query["type"];
        this.slimBarService.complete();
    };
    return InfoWrapperComponent;
}());
InfoWrapperComponent = __decorate([
    core_1.Component({
        selector: "info-wrapper",
        templateUrl: "../resource/templates/info-wrapper.component.html",
        styleUrls: ["../resource/styles/info-wrapper.component.css"]
    }),
    __metadata("design:paramtypes", [router_1.ActivatedRoute,
        ng2_slim_loading_bar_1.SlimLoadingBarService])
], InfoWrapperComponent);
exports.InfoWrapperComponent = InfoWrapperComponent;
//# sourceMappingURL=info-wrapper.component.js.map